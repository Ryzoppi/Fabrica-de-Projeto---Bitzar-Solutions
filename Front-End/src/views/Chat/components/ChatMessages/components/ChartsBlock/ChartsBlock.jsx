...
const DEFAULT_CHART_CONFIG = {
  ...
  tooltip: { theme: 'dark' },
  legend: { labels: { colors: '#aaaaaa' } },
  xaxis: { labels: { style: { colors: '#aaaaaa' } } },
  yaxis: { labels: { style: { colors: '#aaaaaa' } } },
}

const mergeChartConfig = (customOptions = {}) => ({
  ...DEFAULT_CHART_CONFIG,
  ...customOptions,
  chart: { ...DEFAULT_CHART_CONFIG.chart, ...customOptions?.chart },
  legend: { ...DEFAULT_CHART_CONFIG.legend, ...customOptions?.legend },
  xaxis: { ...DEFAULT_CHART_CONFIG.xaxis, ...customOptions?.xaxis },
  yaxis: { ...DEFAULT_CHART_CONFIG.yaxis, ...customOptions?.yaxis },
})
...
const SortableChart = ({
  chart, allCharts, onApply, cols, onColorChange, onFilter, activeFilter,
}) => {
...
const ChartsBlock = ({ charts: initialCharts }) => {
  ...
  const handleColorChange = (chartId, colorIndex, newColor) => {
    setCharts((prev) =>
      prev.map((chart) => {
        ...
        const totalColors =
          chart.categories?.length || chart.options?.labels?.length || chart.series?.length || 0
        const colors = Array.from(
          { length: totalColors },
          (_, i) => chart.options?.colors?.[i] ?? DEFAULT_COLORS[i % DEFAULT_COLORS.length],
        )
        colors[colorIndex] = newColor
        return { ...chart, options: { ...chart.options, colors } }
      }),
    )
  }
    ...
    setCharts((prev) => {
      const updated = prev.map((c) => {
        ...
        if (!currentIsPolar && nextIsPolar) {
          ...
          return {
            ...c, type: newType, series,
            _originalSeries: c._originalSeries ?? c.series,
            options: mergeChartConfig({
              ...c.options, colors: existingColors, labels: categories,
              chart: { ...c.options?.chart, type: newType }, xaxis: {},
            }),
          }
        }
        ...
        if (currentIsPolar && !nextIsPolar) {
          ...
          const series = c._originalSeries
            ? c._originalSeries.map((s) => ({
                ...s,
                data: s.data.map((p) => typeof p === 'object' ? (p.y ?? p) : p),
              }))
            : [{ name: 'Valor', data: Array.isArray(c.series) ? c.series : [] }]
          const { labels: _r, ...cleanOpts } = c.options ?? {}
          return {
            ...c, type: newType, series, _originalSeries: undefined,
            options: mergeChartConfig({
              ...cleanOpts, colors: existingColors,
              chart: { ...cleanOpts?.chart, type: newType }, xaxis: { categories },
            }),
          }
        }
        ...
        return {
          ...c, type: newType,
          options: mergeChartConfig({
            ...c.options, colors: existingColors,
  ...
  // Filtra os dados para exibição sem modificar o estado base
  const displayCharts = activeFilter
    ? charts.map((chart) => {
        const cats = getCats(chart)
        const idx = cats.findIndex(
          (c) => String(c).toLowerCase() === String(activeFilter).toLowerCase(),
        )
        if (idx === -1) return chart

        const polar = ['pie', 'donut'].includes(chart.type)

        if (polar) {
          return {
            ...chart,
            series: [chart.series[idx] ?? 0],
            options: mergeChartConfig({
              ...chart.options,
              labels: [cats[idx]],
              colors: chart.options?.colors ? [chart.options.colors[idx]] : undefined,
            }),
          }
        }

        const filteredSeries = (chart.series ?? []).map((s) => ({
          ...s,
